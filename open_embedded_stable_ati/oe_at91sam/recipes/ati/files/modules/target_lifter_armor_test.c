...
#include "target.h"
#include "target_lifter_armor_test.h"
//---------------------------------------------------------------------------

#define TARGET_NAME		"armor_test lifter"
#define LIFTER_TYPE  	"armor_test"
...
static int hardware_motor_on(int direction)
    {
    unsigned long flags;
    // with the armor_test lifter we don't care about direction,
...
static int hardware_init(void)
    ...
    status = request_irq(INPUT_LIFTER_POS_DOWN_LIMIT, (void*)down_position_int, 0, "armor_test_target_down", NULL);
    if (status != 0)
        {
        if (status == -EINVAL)
            {
    ...
    status = request_irq(INPUT_LIFTER_POS_UP_LIMIT, (void*)up_position_int, 0, "armor_test_target_up", NULL);
    if (status != 0)
        {
        if (status == -EINVAL)
            {
...
//---------------------------------------------------------------------------
// Defines the attributes of the armor_test target lifter for sysfs
//---------------------------------------------------------------------------
static const struct attribute * armor_test_lifter_attrs[] =
    {
    &dev_attr_type.attr,
...
//---------------------------------------------------------------------------
// Defines the attribute group of the armor_test target lifter for sysfs
//---------------------------------------------------------------------------
const struct attribute_group armor_test_lifter_attr_group =
    {
    .attrs = (struct attribute **) armor_test_lifter_attrs,
    };

//---------------------------------------------------------------------------
// Returns the attribute group of the armor_test target lifter
//---------------------------------------------------------------------------
const struct attribute_group * armor_test_lifter_get_attr_group(void)
    {
    return &armor_test_lifter_attr_group;
    }

//---------------------------------------------------------------------------
// declares the target_device that is added/removed through target.ko
//---------------------------------------------------------------------------
struct target_device target_device_lifter_armor_test =
    {
    .type     		= TARGET_TYPE_LIFTER,
    .name     		= TARGET_NAME,
    .dev     		= NULL,
    .get_attr_group	= armor_test_lifter_get_attr_group,
    };
...
static void position_change(struct work_struct * work)
	{
	target_sysfs_notify(&target_device_lifter_armor_test, "position");
	}

//---------------------------------------------------------------------------
// init handler for the module
//---------------------------------------------------------------------------
static int __init target_lifter_armor_test_init(void)
    {
    int retval;
    ...
    hardware_init();
    retval=target_sysfs_add(&target_device_lifter_armor_test);
...
//---------------------------------------------------------------------------
static void __exit target_lifter_armor_test_exit(void)
    {
	hardware_exit();
    target_sysfs_remove(&target_device_lifter_armor_test);
    }
...
// module declarations
//---------------------------------------------------------------------------
module_init(target_lifter_armor_test_init);
module_exit(target_lifter_armor_test_exit);
MODULE_LICENSE("proprietary");
MODULE_DESCRIPTION("ATI target lifter armor_test module");
MODULE_AUTHOR("jpy");
