...
int main(int argc, char **argv) {
   ...
   while((opt = getopt(argc, argv, "hHERv:m:r:i:p:f:s:d:D:t:q:Q:l:L:x:X:F:C:c:j:J:e:")) != -1) {
      switch(opt) {
         case 'h':
            print_help(0);
            break;
    ...
    ****************************************************************/
	...
	if(smartqueue==1) {
		//MCP forks a child process containing the smartqueue program.
		//For compatibility, RF_sock will attach to the child process
		...
		if (child) {
			...
			RF_sock=rf_pair.child;
			//write(RF_sock,"Hello ",7);
		} else {
			/* This is the child. */
			// install signal handlers
			//signal(SIGINT, quitproc);
			...
			if(RF_sock < 0) { PERROR("socket() failed");}
			...
			if (result<0){
				...
				//DCMSG(RED,"Smartqueue RF Master server not found! connect(%s:%d) error: %s  ", inet_ntoa(RF_addr.sin_addr),htons(RF_addr.sin_port),buf);
				EXIT(-1);
			}

			// do child thread here...
			smartqueue_rcc_thread(rf_pair.parent, RF_sock);

			DDCMSG(D_MINION|D_VERY,BLACK,"Smartqueue: minion_thread(...) returned. that minion must have died, so do something smart here like remove it");
			EXIT(0); // if we come back , bail to another level or something more fatal
		}
	} else {
		//legacy approach as of 12/2012
		//mcp owns the socket to RF master
		RF_sock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
		if(RF_sock < 0) { PERROR("socket() failed");}
   ...
   while(!close_nicely) {
      ...
      DDCMSG(D_TIME,RED,"MCP: Waiting for Data with timeout=%d  slave_hunting=%d low=%x hunttime=%d, dwait=%5ld.%09ld, delta=%i, timeout-delta=%i", timeout, slave_hunting, low, hunttime, dwait_time.tv_sec, dwait_time.tv_nsec, delta, timeout-delta);
                  ...
                  if (addr_cnt<max_addr && addr_pool[addr_cnt].inuse &&
                     ...
                     if (addr_cnt>2046){
                        ...
                        if (minions[mID].S.dev_type == Type_MIT) {
                           ...
                           minions[mID].S.position.data = max(0, min(minions[mID].S.position.data, mat_length));           } else {
