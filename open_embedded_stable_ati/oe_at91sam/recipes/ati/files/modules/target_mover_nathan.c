...
//            }
       ...
       if (mover_type == IS_MIT) {
          pid_effort = max(min(pid_effort, 1000), atomic_read(&movement_atomic) == MOVER_DIRECTION_FORWARD ? 20 : 20); // non-zero min to always be touching the throttle line, so the motor controller doesn't reset its timer, and be different forward/reverse to compensate for the gear box
       } else if (mover_type == IS_MAT) {
          pid_effort = max(min(pid_effort, 1000), 20); // always touch line to not engage breaks
       } else {
          pid_effort = max(min(pid_effort, 1000), 0); // min of 0, max of 1000
       }
    }
