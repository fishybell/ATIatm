...
static int MIN_SPEED[] = {15,10,3,10,0}; // minimum acceptible input speed (15 =1 1/2 mph, 20 = 2 mph)
...
static int PID_KP_MULT[]   = {1, 2, 9, 1, 0}; // proportional gain numerator
static int PID_KP_DIV[]    = {4, 15, 3, 3, 0}; // proportional gain denominator
static int PID_KI_MULT[]   = {15, 1, 1, 15, 0}; // integral gain numerator
static int PID_KI_DIV[]    = {475150, 184320, 3, 47515, 0}; // integral gain denominator
static int PID_KD_MULT[]   = {190060, 32768, 250, 190060, 0}; // derivitive gain numerator
static int PID_KD_DIV[]    = {15, 15, 1, 15, 0}; // derivitive gain denominator
#ifdef TESTING_MAX
static int MIN_EFFORT[]    = {10000, 10000, 10000, 10000, 0}; // minimum effort given to ensure motor moves
#else
...
static int MAX_ACCEL[]     = {500, 10000, 10000, 10000, 0}; // maximum effort change in one step
static int MAX_DECCEL[]     = {500, 10000, 10000, 10000, 0}; // maximum effort change in one step
...
static void pid_step_randy(void); 
...
//---------------------------------------------------------------------------
static int percent_from_speed(int speed);	// 10000 * percent, calculated
...
static void speed_timeout_start(int ms, int speed)
	{
	   if (speed != 0){
   	      if (ms <= 0) {
               ms = SPEED_TIMEOUT_IN_MILLISECONDS;
...
#endif
      if (atomic_read(&pid_set_point) != 0){
	pid_ticks ++;
	if (1 || pid_ticks > 6){
      	   pid_step_randy();
	}
...
irqreturn_t quad_encoder_int(int irq, void *dev_id, struct pt_regs *regs)
    ...
    // not used? -- u32 rb, cv;
    int dn1, dn2, oc;
   SENDUSERCONNMSG( "randy quad_encoder_int");
    if (!atomic_read(&full_init))
        {
        return IRQ_HANDLED;
        }
...
// new method
    ...
    if ( status & ATMEL_TC_LDRAS ) {
        ...
        if (CONTINUE_ON[mover_type] & 2) {
            timeout_timer_stop();
            timeout_timer_start(1);
   SENDUSERCONNMSG( "randy reset timer");
        }
...
static int get_pwm_effort(int effort, int error) {
    // figure out max effort
    int max = 100;
    int min = 100;
    int dir = atomic_read(&movement_atomic);
    if (dir == MOVER_DIRECTION_FORWARD) min += 20;
    if (pid_ticks > 14) max = 1000;
    else if (pid_ticks > 12) max = 1000;
    else if (pid_ticks > 10) max = 1000;
    else if (pid_ticks > 8) max = 1000;
    else max = 1000;
    // limit to max effort
    if (effort > max) {
        effort = max;
...
static int current_pid_speed10() {
...
static int calcMoverSpeed( int speed ){
   int rtnSpeed, tmpSpeed, selectSpeed;
   rtnSpeed = 0;
   if (speed != 0){
...
int mover_set_continuous_move(int c) {
   ...
   dock_timeout_stop();
   if (c != 0) {
      speed = abs(calcMoverSpeed(c));
      atomic_set(&continuous_speed, speed);
      if (isMoverAtDock()){
...
void mover_go_home(void) {
   ...
   if (mover_type == IS_MIT){
      if (home_loc == 1) { // Dock at end away from home
         if (atomic_read(&last_sensor) != MOVER_SENSOR_END){
               mover_speed_set(14); // Get there kind of fast
         } else {
...
// new method
static void pid_step_randy() {
    int new_speed=1, input_speed=0, error_sum=0, i, pid_effort=0, pid_p=0, pid_i=0, pid_d=0;
   int direction, targetRatio, delta;
   struct timespec time_now = current_kernel_time();
   struct timespec time_d = timespec_sub(time_now, time_start);
    ...
    new_speed = atomic_read(&pid_set_point); // read desired pid set point
    input_speed = atomic_read(&pid_vel); // read speed from quad_encoder
    direction = atomic_read(&quad_direction);
    delta = atomic_read(&delta_t);

    // read input speed (adjust speed10 value to 1000*percent)
    input_speed = (1000 * abs(current_pid_speed10())) / NUMBER_OF_SPEEDS[mover_type];
    if (!spin_trylock(&pid_lock)) {
        return;
    }
    atomic_set(&pid_vel, 0);

    if(last_pid_speed > 0 && input_speed > (2*new_speed) && input_speed > (2*last_pid_speed)){
        input_speed = last_pid_speed;
    }
    pid_error = new_speed - input_speed; // set point - input
    last_pid_speed = input_speed;
...
*/
    ...
    if (new_speed == 0) {
      ...
      if (mover_type == IS_MIT) {
//         if (pid_effort < 210) pid_effort = 210;
#if 0
         if (new_speed < 50) {
            if (pid_effort < 0) {
               pid_effort = 100;
