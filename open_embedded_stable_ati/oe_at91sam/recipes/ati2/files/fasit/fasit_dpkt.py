...
class _MetaPacket(type):
    def __new__(cls, clsname, clsbases, clsdict):
        ...
        if st:
            ...
            t.__hdr_range__ = dict(zip(t.__hdr_fields__, [ x[3] for x in st ]))
        return t
...
class Packet(object):
    ...
    __hdr__ should be defined as a list of (name, structfmt, default, [low_range, high_range]) tuples
    __byte_order__ can be set to override the default ('>')

    Example::

    >>> class Foo(Packet):
    ...   __hdr__ = (('foo', 'I', 1, [1,2]), ('bar', 'H', 2, [2,20]), ('baz', '4s', 'quux', None))
    ...
    >>> foo = Foo(bar=3)
    >>> foo
    Foo(bar=3)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k in self.__hdr_fields__:
            ...
            range = self.__hdr_range__[k]
            if (range is not None):
                if ((field_value < range[0]) or (field_value > range[1])):
                    self.range_error = True
                    self.logger.debug(k + ' - range error ' + str(field_value) + " " + str(range))
